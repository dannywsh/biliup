...
## 0.0.14
- 更新时间：2026.09.24
- 🔧修复多商品 `goods attach` 请求结构错误：改用 `fromType=7`，并为每个商品分别生成视频框下展示位。

## 0.0.13
- 更新时间：2026.09.20
- 💡同步上游：投稿支持新版分区 `tid_v2`（`--tid-v2`），与旧版 `--tid` 可同时使用。
- 🔧同步上游录播修复：斗鱼空 PlayResponse、streamlink FLV 前缀与失败退避、边录边传测速超时、文件名含点、定时发布时间戳、虎牙弹幕、日志脱敏。
- 🔧同步上游 WebUI：保存配置覆写时保留投稿模板关联。
- 🔒保留本分支 `cover43`、`--post-upload-goods`、`goods`、`season`、`top-reply` 与 fork README。

## 0.0.10
- 更新时间：2026.09.14
- 💡新增 `season` CLI，支持查看新版合集、查看分区视频、添加、移除和排序。
- 🔒合集写操作默认 dry-run，必须显式传入 `--execute` 才会修改合集。
- 🔒登录成功日志不再输出账号名称，避免真实凭据测试泄露个人信息。
- 🧪使用脱敏单元测试和真实账号只读 `season list` 验证 CLI。

## 0.0.8
- 更新时间：2026.09.11
- 💡`goods attach` 支持一次挂载多个商品，并合并为一条评论蓝链。
- 💡多商品评论蓝链支持文案自动换行，单条评论最多挂载 20 个商品。
- 🔧修复多商品挂载被拆分为多条评论的问题。
- 🧪补充多商品评论蓝链请求体和参数解析测试。

## 0.0.7
- 更新时间：2026.09.09
- 💡`goods search` 增加会员购公开详情：品牌、分类、属性、价格摘要和商品图片。
- 💡`goods search` 支持票务详情页，并返回城市、场馆、演出日期、票价、商家、简介和封面。
- 💡纯数字商品 ID 支持会员购识别失败后回退到会员购公开详情和票务详情。
- 🔧保留原有商品识别字段，确保 `goods attach` 的会员购挂载流程兼容。
- 🧪补充会员购/票务详情解析测试。
