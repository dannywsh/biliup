...
- 同步上游：投稿支持新版分区 `tid_v2`（`--tid-v2`）。
- 同步上游录播与 WebUI 修复（斗鱼、streamlink、边录边传、文件名含点、定时发布、虎牙弹幕、日志脱敏、投稿模板保留）。
- 保留本分支 `cover43`、带货、合集与评论置顶能力。

## 0.0.12

- 新增 `season create`，支持通过 B 站创作中心接口创建新版合集。
- 创建合集默认 dry-run，使用 `--execute` 才会提交。
